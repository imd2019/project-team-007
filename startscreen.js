...
export default class Startscreens extends Button {
 ...
 display() {
  ...
  mouseClicked() {
    if(this.infobutton.hitTest(mouseX,mouseY) && this.startscreen) { //wenn deine booleans auf true geptüft werden sollen reicht es einfach den Namen zu schreiben
      clear();// cleart den letzten Screen um diese hässlon Ränder zu vermeiden
      this.startscreen = false;//Änderung der booleans bei click, nicht bei display, sonst spamt es :D
    ...
    if (this.backbutton.hitTest(mouseX,mouseY) && (this.infoscreen || this.chosescreen) ) {
      clear();
      this.back = true;
    }
    
    // hier wird Bedingung noch geändert(Charakter-Auswahl und BGE-Mode), dann sollte es bug-frei funktionieren, sonst switcht es instant zu choosescreen=true
